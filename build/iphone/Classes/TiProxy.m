...
//Rare exceptions to indicate a bug in the _miniproject code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_miniproject = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_miniproject]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.miniproject.module"
