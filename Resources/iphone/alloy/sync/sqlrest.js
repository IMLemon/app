...
function Sync(method, model, opts) {
    function saveData(data) {
        ...
        if (_.isArray(data)) {
            ...
            for (var i in data) _.isUndefined(data[i][model.deletedAttribute]) || true != data[i][model.deletedAttribute] ? -1 != _.indexOf(currentModels, data[i][model.idAttribute]) ? updateSQL(data[i]) : createSQL(data[i]) : deleteSQL(data[i][model.idAttribute]);
        } else {
